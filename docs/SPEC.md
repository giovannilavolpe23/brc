...
## Títulos (implementado — estructura de navegación)
...
Por ahora está implementada únicamente la **estructura visual y de
navegación**: sin cálculo de títulos todavía (ningún dato,
`localStorage` ni lógica nueva). Queda preparada para conectar esa
lógica más adelante sin rehacer el andamiaje de pantallas/rutas.
...
1. **Títulos por estadística** (`#/titulos-estadistica`) — la única
   de las 3 con pestañas **Día / Total** (mismo componente
   `.stats-tabs`/`.stats-tab` que usa Estadísticas, con su propio
   estado independiente `titulosEstadisticaTab`), porque sus títulos
   van a calcularse sobre la misma fuente de datos que Estadísticas:
   un día cerrado del viaje o el acumulado total. Por ahora cada
   pestaña muestra un estado vacío explicativo
   (`.stats-empty-banner`, mismo componente que usa Estadísticas
   cuando todavía no hay días cerrados).
2. **Títulos por encuesta** (`#/titulos-encuesta`) — placeholder
   "Próximamente" (mismo componente `.feature-card.locked` +
   `.soon-tag` que ya usaban las tarjetas bloqueadas de
   Estadísticas). Pensada para títulos votados por los participantes
   mediante alguna encuesta futura.
3. **Títulos por racha** (`#/titulos-racha`) — mismo tratamiento de
   placeholder que Por encuesta. Pensada para títulos por días
   consecutivos cumpliendo un hábito (ej. rachas de Registro diario).
...
- Cálculo real de los títulos en las 3 subsecciones (por estadística,
  por encuesta, por racha).
- Mecanismo de encuestas para "Títulos por encuesta".
