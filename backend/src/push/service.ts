...
export type PushService = {
  ...
  sendDailyRemindersForDate(dateKey: string, source?: "cron" | "daily-fallback"): Promise<DailyReminderSummary>;
...
export function createPushService(
  ...
  async function sendDailyRemindersForDate(
    dateKey: string,
    source: DailyReminderSummary["source"] = "cron"
    ...
    console.info(
      `[push] daily-reminder source=${summary.source} dateKey=${summary.dateKey} activeUsers=${summary.activeUsers} missingDaily=${summary.missingUsers} subscribedUsers=${summary.subscribedUsers} sent=${summary.sent} failed=${summary.failed}`
    );
    return summary;
  }
  ...
  return {
    ...
    sendDailyRemindersForDate,

    async sendDailyReminders(now = new Date()) {
      return sendDailyRemindersForDate(dateKeyDaysBeforeArgentina(1, now), "cron");
    },
