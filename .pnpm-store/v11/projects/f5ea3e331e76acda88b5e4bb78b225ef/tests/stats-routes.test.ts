...
const jereAppearance = {
  preset: "aurora" as const,
  primaryColor: "#4CC9F0",
  secondaryColor: "#7B61FF",
  gradientDirection: "135deg" as const,
...
const statsData: StatsData = {
  users: [
    ...
    { id: jere.id, legacyId: "jere", displayName: "Jere", appearance: jereAppearance },
  ],
...
describe("stats routes", () => {
  it("returns the same global stats to different authenticated users", async () => {
    ...
    assert.deepEqual(gioResponse.body.users.find((user: { legacyId: string }) => user.legacyId === "jere").appearance, jereAppearance);
  });
